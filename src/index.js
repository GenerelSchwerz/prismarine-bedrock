...
// Modified to use a single chat handler with switch statement.
// Commands: !lookAt <x> <y> <z>, !lookAtMe
...
const options = {
  ...
  offline: true,
  version: '1.21.130'
  // ... any other bedrock-protocol options
};

const bot = new BotState(options);

// -----------------------------------------------------------------------
// Helper: compute yaw/pitch to look at a target position from an observer position
// -----------------------------------------------------------------------
...
bot.on('chat', async (data) => {
  const { sourceName, message, rawPacket } = data;

  // Only handle messages starting with '!'
  if (!message.startsWith('!')) return;
  if (sourceName === bot.client.username) return;

  // Parse: strip '!', split on spaces, destructure command and rest
  ...
  switch (cmd.toLowerCase()) {
    ...
    // ── Look at a specific Vec3 coordinate ──
    case 'lookat': {
      if (args.length < 3) {
...
// -----------------------------------------------------------------------
// Helpers moved from chatCommands.js
// -----------------------------------------------------------------------
